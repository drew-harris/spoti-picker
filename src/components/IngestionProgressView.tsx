import type { IngestionProgress } from "../albumIngestion";

export const IngestionProgressView = ({ progress }: { progress: IngestionProgress }) => {
  const percentage = progress.totalAlbums > 0 
    ...
    <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-4 mb-4">
      <div className="flex justify-between items-center mb-2">
        ...
        <span className={`px-2 py-1 rounded text-xs font-medium ${
          ...
          progress.status === 'failed' ? 'bg-red-600 text-white' :
          progress.status === 'in_progress' ? 'bg-blue-600 text-white' :
      ...
      <div className="mb-2">
        <div className="flex justify-between text-sm text-neutral-400 mb-1">
          <span>Progress</span>
          <span>{progress.processedAlbums} / {progress.totalAlbums}</span>
        </div>
